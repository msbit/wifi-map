...
const weightedAveragePercentage = (observations: Observation[]) => {
  return observations.map(o => o.percentage / o.radius).reduce(sum) / observations.map(o => 1 / o.radius).reduce(sum);
};

const sum = (a: number, b: number) => a + b;

const observationContains = (observation: Observation, lat: number, lng: number) => {
...
const subtendedLat = (distance: number, polarRadius = 6356752) => (distance * 180) / (Math.PI * polarRadius);
